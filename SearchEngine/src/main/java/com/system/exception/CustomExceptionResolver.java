...
//            }
            	...
            	return jsonResult(Result.STATUS_FAILED, "用户不存在");
            }
        	else if (ex instanceof IncorrectCredentialsException) {
            	return jsonResult(Result.STATUS_FAILED, "密码错误");
            }
        	else if (ex instanceof UsernamePasswordException) {
				return jsonResult(Result.STATUS_FAILED, ex.getMessage());
			}
